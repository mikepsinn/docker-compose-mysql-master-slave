...
#mysql --host $MYSQL_MASTER_HOST -u$MYSQL_MASTER_USER -p$MYSQL_MASTER_PASSWORD -AN -e "CREATE USER '$MYSQL_REPLICATION_USER'@'%';"
#mysql --host $MYSQL_MASTER_HOST -u$MYSQL_MASTER_USER -p$MYSQL_MASTER_PASSWORD -AN -e "GRANT REPLICATION SLAVE ON *.* TO '$MYSQL_REPLICATION_USER'@'%' IDENTIFIED BY '$MYSQL_REPLICATION_PASSWORD';"
#mysql --host $MYSQL_MASTER_HOST -u$MYSQL_MASTER_USER -p$MYSQL_MASTER_PASSWORD -AN -e 'flush privileges;'


echo "* Set MySQL01 as master on MySQL02"

MYSQL01_Position=$(eval "mysql --host $MYSQL_MASTER_HOST -u$MYSQL_MASTER_USER -p$MYSQL_MASTER_PASSWORD -e 'show master status \G' | grep Position | sed -n -e 's/^.*: //p'")
MYSQL01_File=$(eval "mysql --host $MYSQL_MASTER_HOST -u$MYSQL_MASTER_USER -p$MYSQL_MASTER_PASSWORD -e 'show master status \G'     | grep File     | sed -n -e 's/^.*: //p'")
MASTER_IP=$(eval "getent hosts $MYSQL_MASTER_HOST|awk '{print \$1}'")
echo $MASTER_IP
mysql --host mysqlslave -uroot -p$MYSQL_SLAVE_PASSWORD -AN -e "CHANGE MASTER TO master_host='$MYSQL_MASTER_HOST', master_port=3306, \
...
#echo $SLAVE_IP
#mysql --host $MYSQL_MASTER_HOST -u$MYSQL_MASTER_USER -p$MYSQL_MASTER_PASSWORD -AN -e "CHANGE MASTER TO master_host='mysqlslave', master_port=3306, \
...
mysql --host $MYSQL_MASTER_HOST -u$MYSQL_MASTER_USER -p$MYSQL_MASTER_PASSWORD -AN -e 'set GLOBAL max_connections=2000';
